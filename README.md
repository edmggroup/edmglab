...
>
> **It is fast, and that is measured rather than asserted.** All seven targets in the architecture's performance budget are met: a first visit is interactive in 0.4 s on a throttled connection, a repeat visit in 0.12 s with no network at all, a view switch in under 20 ms, and a 50,000-row cycler export reaches its first plot in 0.6 s. The shell is 147 KB uncompressed against a 150 KB budget. `tools/perf-audit.mjs` re-checks every row.
>
> **Offline now means the whole platform, not just the pages you happened to open.** A few seconds after your first visit the app caches every content file and the chart library, so a phone that has opened EDMGLAB once works completely in a basement lab. `#/health` shows what is really in the cache. **Accessibility was audited across all 207 routes in both themes** and every finding fixed: contrast, focus containment, heading order, control labels, target size, and a type floor no text falls below.
>
> **Electrode materials, with no capacity written down anywhere.** 24 materials across lithium, sodium, potassium, magnesium, zinc, lead and nickel — aqueous and non-aqueous. Each record declares a formula unit and an electron count; the app computes `Q = n·F/(3.6·M)` and prints the arithmetic beside the answer, so there is no stored number that can be wrong. Reported capacities, first-cycle efficiencies and cycle life are absent on purpose, because they mean nothing without the conditions they were measured under. A demonstration takes one pair of materials and produces the three different capacities the field quotes interchangeably — the half-cell number, the balanced full-cell number, and the cell-level number — which differ by a factor of seven before a single measurement has been made.
>
> **And a panel that answers *why* some of them work in water.** Water is stable over only 1.229 V, and that window slides down 59 mV per pH unit without ever getting wider. Drag the pH slider and watch zinc cross the hydrogen line at pH 12.9 — which is the whole reason alkaline zinc cells have been made for a century while mildly acidic ones are still a research problem. The sixteen standard potentials it is built on are the only numbers in the module the app did not compute, and every one carries a citation. You can put your own measured potential on the plot; EDMGLAB will not supply one it has not verified.
>
> **And a page that walks the whole route, for one material at a time.** This platform was first described not as a set of modules but as a pathway — concept, chemistry, material, preparation, characterisation, fabrication, testing, calculation, analysis, interpretation, troubleshooting. Every module got built and none of them was that, because a sidebar sorts things by what they are and an experiment needs them sorted by when you need them. `#/pathway` picks a material and fills in all eleven stages with what the platform actually holds for it — **and says plainly where it holds only the generic module.** It counts those gaps and shows the count, per material and across all of them, because that count is the honest map of what this group still has to write down.
>
> The shared foundation — application shell, animation engine, diagram engine, enforced simulation labelling, chart layer, expression/unit engine, CSV parser and data health check — is in place. **Every module in the roadmap is now built**; the sidebar carries no phase badges.
>
...
>
> **An optional 4-digit PIN gate, and an admin page that can actually grant access.** Off by default. Turn it on from Menu → Access Control and choose one shared PIN or a PIN per person — the second also keeps each person's progress separate on a shared lab machine. PINs are stored as PBKDF2 hashes, never as numbers. By default an admin generates the configuration and commits it; deploy the small Google Apps Script in `docs/apps-script/` and the admin can instead **add, suspend or remove somebody from the page itself**, live for everyone on their next visit, with an audit log of every change. The admin key lives on Google's servers, never in this repository. **It is a soft gate, not security** — the site is static, so anyone with the URL can still open any data file directly, and the app says so plainly in three places rather than letting anyone believe otherwise.
>
> **All scientific content is draft**, marked as such in the interface, and pending review by the research group. **There is now machinery for fixing that.** `node tools/export-review.mjs` writes a Word document per module — every entry with its full text, its id and a tick box — to circulate and mark up. Menu → **Content Review**, open to everyone in the group, records the verdicts against the same 285 entries, with a running count of what is still unchecked and three answers rather than two: *correct as written*, *needs a change*, and *I cannot judge this* — the last being the one that tells you an entry needs a different reviewer. Deploy the review script and several people can work at once and see each other's disagreements; nothing is ever overwritten. When the group signs an entry off, its page stops saying draft and says **checked**, with who and when. **Start with the safety section, with your safety officer**, before anyone treats it as guidance.
