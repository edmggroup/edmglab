...
| `dashboard-test.mjs` | 13 assertions on the landing page. It spent months announcing a "Phase 0 build" whose content "arrives module by module", under a *Coming next* table listing five modules finished weeks earlier, with fourteen cards reading **Phase undefined** — and every other audit passed the whole time, because a page can be accessible, fast, offline-capable and completely wrong. Checks that the roadmap table and phase chips stay gone, that the remaining-work section is derived from the data files, and that **every built module has a description** — the one list here that cannot be derived. No browser, no server |
